...
// // import static common.utils.CommonUtil.listOrEmpty;
// // import static common.utils.CommonUtil.nullOrEmpty;
// // import static service.security.JwtFilter.EMAIL_CLAIM_KEY;
// // import static service.security.JwtFilter.USERNAME_CLAIM_KEY;
// // import static service.security.SecurityUtil.findEmailFromClaims;
// // import static service.security.SecurityUtil.getClaimOrObject;
// // import static service.security.SecurityUtil.getFirstMatchJwtClaim;
// // import static service.security.SecurityUtil.writeJsonResponse;
// // import static service.util.UserUtil.getRoleListFromUser;
...
// // import schema.api.security.AuthenticationConfiguration;
// // import schema.api.security.AuthorizerConfiguration;
// // import schema.auth.JWTAuthMechanism;
// // import schema.auth.ServiceTokenType;
// // import schema.entity.teams.User;
// // import schema.security.client.OidcClientConfig;
// // import schema.type.Include;
// // import service.Entity;
// // import service.auth.JwtResponse;
// // import service.security.jwt.JWTTokenGenerator;
// // import service.util.JsonUtils;
...
// "CallbackUrl",
// authenticationConfiguration.getOidcConfiguration().getCallbackUrl());
// CommonHelper.assertNotBlank(
// "ServerUrl",
// authenticationConfiguration.getOidcConfiguration().getServerUrl());

// // Build Required Params
// this.client =
// buildOidcClient(authenticationConfiguration.getOidcConfiguration());
...
// this.promptType =
// authenticationConfiguration.getOidcConfiguration().getPrompt();
// }
...
// for (int j = 1; j <= 5; ++j) {
// if
// (clientConfig.getCustomParams().containsKey(String.format("customParamKey%d",
// j))) {
...
// oidcClient.getConfiguration().getCustomParams().put("access_type",
// "offline");
// } else {
...
// LOG.debug("Performing Auth Login For User Session: {} ",
...
// public static void checkAndStoreRedirectUriInSession(HttpServletRequest
// request) {
...
// {
// try {
// LOG.debug("Performing Auth Callback For User Session: {} ",
// req.getSession().getId());
...
// LOG.error(
// "Bad authentication response, error={}",
...
// client.getConfiguration().getProviderMetadata();
...
// LOG.error("Refresh token is null for user session: {}",
// req.getSession().getId());
// }
...
// try {
// LOG.debug(
// "Performing Auth Refresh For User Session: {} ",
// httpServletRequest.getSession().getId());
// Optional<OidcCredentials> credentials =
// getUserCredentialsFromSession(httpServletRequest);
// if (credentials.isPresent()) {
// LOG.debug(
// "Credentials Found For User Session: {} ",
// httpServletRequest.getSession().getId());
...
// private String buildLoginAuthenticationRequestUrl(final Map<String, String>
// params) {
...
// }
// return
// client.getConfiguration().getProviderMetadata().getAuthorizationEndpointURI().toString()
// + '?'
// + queryString;
// }
...
// authParams.put(OidcConfiguration.SCOPE,
// client.getConfiguration().getScope());
// authParams.put(OidcConfiguration.RESPONSE_TYPE,
// client.getConfiguration().getResponseType());
// authParams.put(OidcConfiguration.RESPONSE_MODE, "query");
// authParams.putAll(client.getConfiguration().getCustomParams());
// authParams.put(OidcConfiguration.CLIENT_ID,
// client.getConfiguration().getClientId());

// return new HashMap<>(authParams);
// }

// private Optional<OidcCredentials>
// getUserCredentialsFromSession(HttpServletRequest request) {
...
// } else {
// if (credentials == null) {
// LOG.error("No credentials found against session. ID: {}",
// request.getSession().getId());
// } else {
// LOG.error("No refresh token found against session. ID: {}",
// request.getSession().getId());
// }
// }
...
// executeAuthorizationCodeTokenRequest(httpServletRequest, request,
// oidcCredentials);
// }
// }
...
// LOG.debug("Request state: {}/response state: {}", requestState,
// responseState);
...
// private OidcCredentials buildCredentials(AuthenticationSuccessResponse
// successResponse) {
...
// private void validateNonceIfRequired(HttpServletRequest req, JWTClaimsSet
// claimsSet)
// throws BadJOSEException {
// if (client.getConfiguration().isUseNonce()) {
// String expectedNonce =
// (String)
// req.getSession().getAttribute(client.getNonceSessionAttributeName());
// if (CommonHelper.isNotBlank(expectedNonce)) {
// String tokenNonce;
// try {
// tokenNonce = claimsSet.getStringClaim("nonce");
// } catch (java.text.ParseException var10) {
// throw new BadJWTException("Invalid JWT nonce (nonce) claim: " +
// var10.getMessage());
// }
...
// retrieveCallbackParameters(HttpServletRequest request) {
...
// private ClientAuthentication getClientAuthentication(OidcConfiguration
// configuration) {
...
// ClientAuthenticationMethod preferredMethod =
...
// } else {
// chosenMethod =
// preferredMethod != null ? preferredMethod :
// ClientAuthenticationMethod.getDefault();
// LOG.info(
// "Provider metadata does not provide Token endpoint authentication methods.
// Using: {}",
// chosenMethod);
// }
...
// } else if
// (ClientAuthenticationMethod.CLIENT_SECRET_BASIC.equals(chosenMethod)) {
// Secret clientSecret = new Secret(configuration.getSecret());
// clientAuthenticationMechanism = new ClientSecretBasic(clientID,
// clientSecret);
...
// throw new TechnicalException(
// "Configured authentication method (" + configurationMethod + ") is not
// supported.");
// }
...
// } else {
// throw new TechnicalException(
// "None of the Token endpoint provider metadata authentication methods are
// supported: "
// + metadataMethods);
// }
// }
...
// LOG.error("[Auth Callback Servlet] Failed in Auth Login : {}",
// e.getMessage());
// resp.getOutputStream()
// .println(
// String.format(
// "<p> [Auth Callback Servlet] Failed in Auth Login : %s </p>",
// e.getMessage()));
// }

// private void sendRedirectWithToken(
// HttpServletRequest request, HttpServletResponse response, OidcCredentials
// credentials)
...
// String email = findEmailFromClaims(claimsMapping, claimsOrder, claims,
// principalDomain);

// String redirectUri = (String)
// request.getSession().getAttribute(SESSION_REDIRECT_URI);
...
// Entity.getUserRepository().updateUserLastLoginTime(user,
...
// private void renewOidcCredentials(HttpServletRequest request, OidcCredentials
// credentials) {
// LOG.debug("Renewing Credentials for User Session {}",
// request.getSession().getId());
// if (client.getConfiguration() instanceof AzureAd2OidcConfiguration
...
// final HttpServletRequest httpServletRequest, final OidcCredentials
// credentials) {
...
// String accessToken = JSONObjectUtils.getString(jsonObjectResponse,
// "access_token");
// LOG.info(
...
// User user = Entity.getEntityByName(Entity.USER, username, "id",
// Include.NON_DELETED);
...
// throw new TechnicalException("No refresh token available to request new
// access token.");
// }
...
// HttpConstants.APPLICATION_FORM_ENCODED_HEADER_VALUE);
// headers.put(HttpConstants.ACCEPT_HEADER, HttpConstants.APPLICATION_JSON);

// URL tokenEndpointURL =
// azureConfig.findProviderMetadata().getTokenEndpointURI().toURL();
// connection = HttpUtils.openPostConnection(tokenEndpointURL, headers);

// String requestBody =
// azureConfig.makeOauth2TokenRequest(refreshToken.getValue());
...
// throw new TechnicalException("Token refresh failed with status: " +
// responseCode);
// }
...
// azureAdProfile.setAccessToken(new BearerAccessToken((String)
// res.get("access_token")));
// azureAdProfile.setRefreshToken(new RefreshToken((String)
...
// } else {
// throw new AuthenticationException("Invalid JWT token, 'username' claim is not
// present");
// }
// } else {
...
// public static void validatePrincipalClaimsMapping(Map<String, String>
// mapping) {
...
// throw new IllegalArgumentException(
// "Invalid JWT Principal Claims Mapping. Both username and email should be
// present");
// }
// }
...
// private HTTPResponse executeTokenHttpRequest(TokenRequest request) throws
// IOException {
...
// final OidcClient client, final HttpServletRequest request, final Map<String,
// String> params) {
...
// request.getSession().setAttribute(client.getStateSessionAttributeName(),
// state);
// }
...
// request.getSession().setAttribute(client.getNonceSessionAttributeName(),
// nonce.getValue());
// }
...
// request.getSession().setAttribute(client.getCodeVerifierSessionAttributeName(),
// verfifier);
// params.put(
...
// HttpServletRequest httpServletRequest, TokenRequest request, OidcCredentials
// credentials) {
// HTTPResponse httpResponse = executeTokenHttpRequest(request);
// OIDCTokenResponse tokenSuccessResponse =
// parseTokenResponseFromHttpResponse(httpResponse);
...
// // Check expiry, azure on first go itself is returning a expried token
// sometimes
// Date expirationTime =
// credentials.getIdToken().getJWTClaimsSet().getExpirationTime();
...
// private OIDCTokenResponse parseTokenResponseFromHttpResponse(HTTPResponse
// httpResponse)
